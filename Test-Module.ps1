...
#!/usr/bin/env pwsh
# .SYNOPSIS
#   profileupdater testScript v0.1.0
# .EXAMPLE
#   ./Test-Module.ps1 -version 0.1.0
#   Will test the module in ./BuildOutput/profileupdater/0.1.0/
# .EXAMPLE
#   ./Test-Module.ps1
...
param (
    ...
    [ValidateScript({
            ...
            $b_Path = [IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'profileupdater')
...
begin {
    $TestResults = $null
    $BuildOutput = [IO.DirectoryInfo]::New([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'profileupdater'))
    ...
    $BuildOutDir = Resolve-Path $([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'profileupdater', $version)) -ErrorAction Ignore | Get-Item -ErrorAction Ignore
    if (!$BuildOutDir.Exists) { throw [System.IO.DirectoryNotFoundException]::new($BuildOutDir) }
    $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir.FullName, "profileupdater.psd1"))
    ...
    if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "profileupdater.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [System.IO.FileNotFoundException]::New("Module manifest file Was not Found in '$($BuildOutDir.FullName)'.") }
    $script:fnNames = [System.Collections.Generic.List[string]]::New(); $testFiles = [System.Collections.Generic.List[IO.FileInfo]]::New()
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'profileupdater.Integration.Tests.ps1')))
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'profileupdater.Features.Tests.ps1')))
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'profileupdater.Module.Tests.ps1')))
}
